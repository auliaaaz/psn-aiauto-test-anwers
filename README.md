...
6. Ensure the CSV file named `device_network_data.csv` and adjust `path_to_file\` directory. If the file is in a different location, update the file path in the script accordingly.

7. Run the python file with:
  ```
  python calculate_downtime.py
  ```

If it run expectedly it will return 58310.0 as the result of total downtime calculation.
